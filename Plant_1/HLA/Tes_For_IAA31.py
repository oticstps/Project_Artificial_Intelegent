...
def parse_message(message):
    if not message:
        return None
    try:
        prefix, data_string = message.split(",", 1)
        data_string = data_string.rstrip('#')
        data_values = [int(value) for value in data_string.split(',') if value]
        return data_values
...
def on_message(client, userdata, msg):
    ...
    if parsed_data and msg.payload.decode('utf-8').startswith("data_reset,"):
        reset_conditions()
...
try:
    while True:
        ...
        try:
            ...
            if hla_count >= 10 and hla_count <= 44 and (time.time() - last_judgment) >= judgment_cooldown:
