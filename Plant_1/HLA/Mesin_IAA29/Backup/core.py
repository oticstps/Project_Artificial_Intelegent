...
client.loop_start()
model = YOLO("/home/otics/on/best_iaa32_nov_10_2024.pt")
class_names = ['hla', 'off', 'altar', 'box_after', 'adm_export', 'tmmin1l', 'tmmin1e']
media_folder = "media"
...
while True:
    ...
    adm_export_count = sum(1 for obj in detected_objects if int(obj[5]) < len(class_names) and class_names[int(obj[5])] == 'adm_export')
    tmmin1l_count = sum(1 for obj in detected_objects if int(obj[5]) < len(class_names) and class_names[int(obj[5])] == 'tmmin1l')
    tmmin1e_count = sum(1 for obj in detected_objects if int(obj[5]) < len(class_names) and class_names[int(obj[5])] == 'tmmin1e')



    if adm_export_count > 0:
        active_part = "ADM Export"
        if total_hla is None:
            total_hla = 108
        ...
        if total_hla is None:
            total_hla = 88
            print("[INFO] TMmin1L detected. Setting total_hla to 88.")
    elif tmmin1e_count > 0:
        active_part = "TMmin1E"
        if total_hla is None:
            total_hla = 80
            print("[INFO] TMmin1E detected. Setting total_hla to 80.")
    else:
        active_part = "None"

    print(f"Detected HLA: {hla_count}, ADM Export: {adm_export_count}, TMmin1L: {tmmin1l_count}, TMmin1E: {tmmin1e_count}")
