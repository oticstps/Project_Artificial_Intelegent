...
def reset_conditions():
    global total_hla, active_part, total_hla_count, previous_hla_count, condition_button, status_text, status_color, last_status, capture_history, last_capture_time, parsed_data, alarm_counter
...
while True:
    ...
    if total_hla is not None and parsed_data is not None and parsed_data[1] == 1:
        ...
        if hla_count == total_hla:
            ...
            if (time.time() - last_capture_time) >= capture_cooldown:
                ...
                capture_image(annotated_frame, timestamp, "OK")
            ...
            ng_counter += 1
            update_capture_history(hla_count, timestamp, status)
            capture_image(annotated_frame, timestamp, "NG")
        parsed_data[1] = 0
        time.sleep(1)

    # update_hla_counts(hla_count)
    ...
    status_y = 90 + 15 + 50  # Move further down
    status_bg_color = (50, 50, 50)  # Solid background color
    if hla_count == 20:
    ...
    for idx, record in enumerate(capture_history):
        ...
        last_status = status  # Update last_status with the current status
    ...
    if last_status == "READY":
        indicator_color = (255, 255, 0)  # Yellow for READY
        indicator_text = "READY"
    else:
        indicator_color = (0, 255, 0) if last_status == "OKE" else (0, 0, 255)  # Green for OKE, Red for NG
        indicator_text = "SUCCESS" if last_status == "OKE" else "FAILURE"
    ...
    if last_status == "OKE":
        ...
        # Standby mode: Green and Red lights off
        cv2.rectangle(annotated_frame, (andon_x, andon_y), (andon_x + andon_width, andon_y + andon_height), (0, 0, 0), -1)
        cv2.rectangle(annotated_frame, (andon_x, andon_y + 2 * (andon_height + 20)), (andon_x + andon_width, andon_y + 3 * andon_height + 40), (0, 0, 0), -1)
