...
# 3. Interlock sound 5 berbunyi berulang sampai reset
# 4. Setelah reset, interlock tidak langsung retrigger jika hla_count masih 44
# rama
...
def reset_conditions():
   ...
   global previous_active_part, mismatch_active
...
def main():
   ...
   global mismatch_active, last_mismatch_sound_time
   global parsed_data
   global judgment_ng_alarm_active, last_judgment_ng_sound_time
   global interlock_active, last_interlock_sound_time, interlock_reset_block
   ...
   try:
       ...
       while True:
           ...
           try:
               ...
               # Sound part change
               if previous_active_part != new_active_part:
                   if new_active_part == "tmmin-1E" and previous_active_part is not None:
                       play_sound(7)
                   elif new_active_part == "tmmin-1L" and previous_active_part is not None:
                       play_sound(8)
                   elif new_active_part == "ADM Export" and previous_active_part is not None:
                       play_sound(9)
               ...
               if mismatch_now and (not mismatch_active) and ((current_time - last_mismatch_sound_time) > mismatch_sound_cooldown):
                   play_sound(4)
                   last_mismatch_sound_time = current_time
                   mismatch_active = True
               elif not mismatch_now:
                   mismatch_active = False


               # =====================================================
               # TRIGGER INTERLOCK
               # Interlock dilatch, bunyi berulang sampai reset.
               # Setelah reset, tidak langsung retrigger kalau count masih 44.
               # =====================================================
               if (not judgment_ng_alarm_active) and (not interlock_active) and (not interlock_reset_block):
                   if hla_count == 44:
               ...
               if judgment_ng_alarm_active:
                   ...
                   if (current_time - last_interlock_sound_time) >= interlock_sound_interval:
                       play_sound(5)
                       last_interlock_sound_time = current_time


               # Jika interlock aktif lalu tray jadi 0
               if hla_count == 0 and condition_button == 1:
                   ...
                   cv2.putText(frame, indicator_text, (frame.shape[1] - 350, 80),
                   ...
                   cv2.putText(frame, ng_proses_text, (frame.shape[1] - 350, ng_proses_y + 50),
                               cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 0, 0), 6)
